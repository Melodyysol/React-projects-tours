...
const Cart = () => {
  ...
  // const [initial, dispatch] = useReducer(reducer, 1)
  const [lists, setLists] = useState<CartItem[]>(carts)
  const totalCost = lists.reduce((total, item) => total + (item.price * item.quantity), 0);
const totalQuantity = lists.reduce((total, item) => total + item.quantity, 0);
  ...
  const increaseCart = (id: number) => {
    setLists(prev =>
      prev.map(cart => cart.id === id ? { ...cart, quantity: cart.quantity + 1 } : cart)
    )
  }
